...
from keras.layers import Reshape, Activation, Conv2D, Input, MaxPooling2D, BatchNormalization, Flatten, Dense, Lambda, TimeDistributed, LSTM
...
class YOLO(object):
    def __init__(self, backend,
        ...
        features = TimeDistributed(self.feature_extractor.feature_extractor)(input_image)

        conv_layer = Conv2D(self.nb_box * (4 + 1 + self.nb_class), 
                        (1,1), strides=(1,1), 
                        padding='same', 
                        name='DetectionLayer', 
        ...
        detection = Reshape((self.sequence_length, self.grid_h * self.grid_w * self.nb_box * (4 + 1 + self.nb_class)))(detection)

        print(self.grid_h * self.grid_w * self.nb_box * (4 + 1 + self.nb_class))


        tt_input_shape = [3, 13, 26, 5]
        ...
        #TT_LSTM(self.grid_h * self.grid_w * self.nb_box *  (4 + 1 + self.nb_class), return_sequences=True)(detection)

        # make the object detection layer
        output = Reshape((self.sequence_length, self.grid_h, self.grid_w, self.nb_box, 4 + 1 + self.nb_class))(detection)
